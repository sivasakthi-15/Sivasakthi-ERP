...
async function extractFirstHeader() {
  ...
  for await (const line of rl) {
    if (line.includes("Corporate Header block") && line.includes("view_file")) {
      ...
      if (parsed.content && parsed.content.includes("Corporate Header block")) {
        console.log("Found a view_file with the header!");
        let idx = parsed.content.indexOf("Corporate Header block");
